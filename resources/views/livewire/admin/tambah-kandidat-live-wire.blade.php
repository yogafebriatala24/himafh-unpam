<div>
    @section('title')
        Tambah Calon
    @endsection
    <div class="card p-2">
        <div class="row">
            <div class="col-8">
                <div>
                    <div>
                        @if ($image === null)
                            <img src="data:image/png;base64,"
                                alt="" class="img-fluid" style="height: 200px">
                        @else
                            <img src="{{ $image->temporaryUrl() }}" alt="" class="img-fluid"
                                style="height: 200px">
                        @endif
                    </div>
                    <div>
                        <span class="text-primary mt-2" wire:loading wire:target="image">
                            <div class="spinner-border" role="status">
                                <span class="sr-only"></span>
                            </div>
                        </span>
                        <input type="file" class="form-control mt-2" wire:model="image" ">
                        @error('image') <span class="     error text-danger"
                                style="font-size: 14px;">{{ $message }}</span>
                        @enderror
                    </div>
                </div>
            </div>

            <div class="col-6">
                <div class="mt-3">
                    <label for="">Pilih dari user</label>
                    <span class="text-primary mt-2 h-2">
                        <div class="spinner-border spinner-border-sm" role="status" wire:loading
                            wire:target="hendleSelect">
                            <span class="sr-only"></span>
                        </div>
                    </span>
                </div>
                <input type="text" wire:model="search" class="form-control" placeholder="cari nama, email, nim calon">
                <div>
                    @if (strlen($search) >= 2 && $isSearch)
                        <span class="text-primary mt-2" wire:loading wire:target="search">
                            <div class="spinner-border" role="status">
                                <span class="sr-only"></span>
                            </div>
                        </span>
                        <div class="list-group">
                            @foreach ($users as $user)
                                <div wire:click="hendleSelect({{ $user->id }})"
                                    class="list-group-item list-group-item-action btn border-b">{{ $user->name }} -
                                    {{ $user->nim }}
                                </div>
                            @endforeach

                        </div>
                    @endif
                </div>
                @error('userId') <span class="    error text-danger" style="font-size: 14px;">{{ $message }}</span>
                @enderror
            </div>
            <div class="col-6">
                <div class="mt-3">
                    <label for="">Jabatan Calon</label>
                </div>
                <select class="form-select" wire:model="role">
                    <option value="">Pilih</option>
                    <option value="Ketua">Calon Ketua</option>
                    <option value="Wakil">Calon Wakil</option>
                </select>
                @error('role') <span class="error text-danger" style="font-size: 14px;">{{ $message }}</span>
                @enderror
            </div>
            <div class="col-12">
                <div class="btn btn-primary text-center btn-block mt-4" wire:click="create">
                    Simpan Data
                </div>
            </div>

        </div>
    </div>


    @push('style')
        <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.1.0/css/bootstrap.min.css">
        <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/font-awesome/4.7.0/css/font-awesome.min.css">
        <link rel="stylesheet" href="{{ asset('livesearch-bootstrap/dist/css/bootstrap-select.css') }}">
    @endpush
    @push('script')
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.1.0/js/bootstrap.bundle.min.js"></script>
        <script src="{{ asset('livesearch-bootstrap/dist/js/bootstrap-select.js') }}"></script>
    @endpush
</div>
